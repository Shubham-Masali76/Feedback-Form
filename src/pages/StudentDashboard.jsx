...
export default function StudentDashboard({ user }) {
    ...
    const fetchSystemStatusAndData = async () => {
      try {
        ...
        staffFeedSnap.forEach(d => {
          ...
          const match = allocData.find(a => 
            a.id === data.allocationId || 
            (a.staff === data.staffName && a.subject === data.subject && String(a.targetClass || a.tClass).trim() === String(data.targetClass).trim())
          );
  ...
  const handleSubmitExitSurvey = async (e, formDoc) => {
    ...
    if (Object.keys(exitRatings).length < formDoc.questions.length) {
      warning(`Please answer all ${formDoc.questions.length} questions before submitting.`);
      return;
    }
    ...
    try {
      ...
      let totalScore = 0;
      Object.values(exitRatings).forEach((val) => (totalScore += parseInt(val)));
  ...
  const handleSubmitInstFeedback = async (e) => {
    ...
    if (Object.keys(instRatings).length < INSTITUTION_QUESTIONS.length) {
      warning(`Please answer all ${INSTITUTION_QUESTIONS.length} questions before submitting.`);
      return;
    }
    ...
    try {
      let totalScore = 0;
      Object.values(instRatings).forEach((val) => (totalScore += parseInt(val)));
      ...
      success(`Institution Feedback for ${currentAcadYear} submitted successfully.`);
      setHasSubmittedInst(true);
      setInstRatings({});
    } catch (error) {
  ...
  return (
    <div className="space-y-8">
      ...
      {isInstPortalOpen && (
        <Card className={`overflow-hidden p-0 ring-1 ${hasSubmittedInst ? 'ring-slate-200/60' : 'ring-amber-200/60 shadow-glow-amber animate-pulse-subtle'}`}>
          <div className="flex flex-col sm:flex-row items-center justify-between p-5 gap-4">
             <div className="flex items-center gap-4">
                <div className={`flex h-12 w-12 items-center justify-center rounded-2xl ${hasSubmittedInst ? 'bg-slate-100 text-slate-500' : 'bg-gradient-to-br from-amber-500 to-orange-600 text-white shadow-lg shadow-amber-200'}`}>
                  <Building2 size={24} strokeWidth={2} />
                </div>
                <div>
                   <h2 className="text-lg font-bold text-slate-900">Institution Level Feedback</h2>
                   <p className="text-xs font-medium text-slate-500 italic">Global Annual Survey • {currentAcadYear}</p>
                </div>
             </div>
             {hasSubmittedInst ? (
                ...
                <button
                 ...
                 >
                   {selectedAllocation === 'institution_survey' ? 'Back to Subjects' : 'Fill Survey now'}
                 </button>
             )}
          </div>
        </Card>
      ...
      <div className="grid lg:grid-cols-3 gap-7">
              ...
              <div className="space-y-6">
                {(() => {
                  const renderButton = (alloc) => {
                    ...
                    const hasExitForm = exitForms.some(f => 
                      f.subject === alloc.subject && 
                      String(f.targetClass || "").trim() === String(alloc.targetClass ?? alloc.tClass ?? "").trim() &&
                      f.staffName === alloc.staff
                    );
                    const isExitCompleted = submittedExitSurveys.includes(alloc.id);
                    ...
                    return (
                      <button
                        ...
                        disabled={isCompleted && (!hasExitForm || isExitCompleted)}
                      ...
                      >
                        <div className="flex justify-between items-center gap-2 mb-2">
                          ...
                          <div className="flex gap-1.5 items-center">
                            {hasExitForm && (
                               <span className={`text-[9px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${isExitCompleted ? 'bg-slate-200 text-slate-500' : (selectedAllocation === alloc.id ? 'bg-blue-800/40 text-blue-100' : 'bg-emerald-100 text-emerald-800 ring-1 ring-emerald-200')}`}>Survey Open</span>
                            )}
                            {isCompleted && (
                              <CheckCircle
                                size={18}
                        ...
                        <h3
                          className={`font-display text-base font-bold leading-tight ${
                            selectedAllocation === alloc.id && !(isCompleted && (!hasExitForm || isExitCompleted))
                              ? "text-white"
                              : "text-slate-900"
        ...
        <div className={selectedAllocation === "institution_survey" ? "lg:col-span-3" : "lg:col-span-2"}>
          <Card className="h-full overflow-hidden p-0 !shadow-soft-lg ring-1 ring-slate-200/50">
            {!selectedAllocation ? (
              <div className="relative flex h-full min-h-[360px] flex-col items-center justify-center overflow-hidden px-6 py-14 text-center">
               ...
               <form onSubmit={handleSubmitInstFeedback} className="flex flex-col animate-in slide-in-from-right duration-500">
                  <div className="border-b border-slate-200/80 bg-gradient-to-r from-amber-600 via-orange-600 to-rose-600 px-6 py-6 md:px-8">
                    ...
                    <p className="mt-2 max-w-xl text-sm leading-relaxed text-amber-100/95">
                      Rate your satisfaction with the institution's facilities and services from{" "}
                  ...
                  <div className="space-y-5 bg-gradient-to-b from-slate-50/80 to-white p-6 md:p-8">
                    {hasSubmittedInst ? (
                      <div className="flex flex-col items-center justify-center py-12 text-center">
                        ...
                        <h3 className="font-display text-2xl font-bold text-slate-800">Feedback Received</h3>
                        <p className="text-slate-500 mt-2 max-w-xs mx-auto">Thank you for your valuable feedback. Your response for {currentAcadYear} has been recorded.</p>
                      </div>
                    ) : (
                        ...
                        >
                          ...
                          >
                            {[1, 2, 3, 4, 5].map((num) => {
                              ...
                              const labels = ["Average", "Satisfactory", "Good", "Very Good", "Excellent"];
                              return (
                                <label
                                  key={num}
                                  title={labels[num-1]}
                                ...
                                >
                                  <input
                                    ...
                                    value={num}
                                    checked={selected}
                                    onChange={() => handleInstRatingChange(idx, num)}
                                    className="sr-only"
                                  />
                                  {num}
                                </label>
                  ...
                  {!hasSubmittedInst && (
                    <div className="sticky bottom-0 border-t border-slate-200/80 bg-white/95 backdrop-blur-md px-6 py-6 md:px-8 flex flex-col sm:flex-row gap-3 sm:gap-4 z-10 shadow-[0_-4px_20px_rgb(0,0,0,0.03)]">
                      <button
                        type="button"
                        disabled={isSubmittingInst || Object.keys(instRatings).length === 0}
                      ...
                      >
                        ...
                        {isSubmittingInst ? "Submitting…" : "Submit Institution Feedback"}
                      </button>
                    </div>
                  )}
                </form>
              ...
              const targetData = allocations.find((a) => a.id === selectedAllocation);
              const targetClass = String(targetData?.targetClass ?? targetData?.tClass ?? "").trim();
              const exitForm = exitForms.find(f => f.subject === targetData?.subject && String(f.targetClass || "").trim() === targetClass && f.staffName === targetData?.staff);
              const isFacultyCompleted = submittedReviews.includes(selectedAllocation);
              const isExitCompleted = submittedExitSurveys.includes(selectedAllocation);
                  ...
                  <div className="space-y-5 bg-gradient-to-b from-slate-50/80 to-white p-6 md:p-8">
                    {isFacultyCompleted ? (
                      <div className="flex flex-col items-center justify-center py-12 text-center">
                        ...
                        <h3 className="font-display text-xl font-bold text-slate-800">Feedback Submitted</h3>
                        <p className="text-slate-500 mt-2 text-sm">Thank you for completing the faculty evaluation.</p>
                      </div>
                    ) : (
              ...
              const renderExitSurvey = () => (
                <form onSubmit={(e) => handleSubmitExitSurvey(e, exitForm)} className="flex flex-col animate-in fade-in duration-300">
                  ...
                  <div className="space-y-5 bg-gradient-to-b from-slate-50/80 to-white p-6 md:p-8">
                    {isExitCompleted ? (
                      <div className="flex flex-col items-center justify-center py-12 text-center">
                        <CheckCircle className="text-emerald-500 mb-3" size={48} />
                        <h3 className="font-display text-xl font-bold text-slate-800">Survey Submitted</h3>
                        <p className="text-slate-500 mt-2 text-sm">Thank you for submitting the Course Exit Survey.</p>
                      </div>
                    ) : (
                              ...
                              return (
                                ...
                                >
                                  <input
                                    ...
                                    value={num}
                                    checked={selected}
                                    onChange={() => handleExitRatingChange(idx, num)}
                                    className="sr-only"
                                  />
                                  {num}
                                </label>
                  ...
                  {!isExitCompleted && (
                    <div className="border-t border-slate-200/80 bg-white px-6 py-6 md:px-8 flex flex-col sm:flex-row gap-3 sm:gap-4">
                      <button
                        type="button"
                        disabled={isSubmittingExit || Object.keys(exitRatings).length === 0}
                      ...
                      >
                        <Send size={20} aria-hidden />
                        {isSubmittingExit ? "Submitting…" : "Submit exit survey"}
                      </button>
                    </div>
                  )}
                </form>
                ...
                <div className="flex flex-col h-full">
                  ...
                  {exitForm && (
                     <div className="border-b border-slate-200/80 bg-slate-50 flex overflow-x-auto">
                        ...
                        >
                           Faculty Feedback
                           {isFacultyCompleted && <CheckCircle size={14} className="inline ml-2 text-emerald-500 mb-0.5" />}
                        </button>
                        <button
                          type="button"
                        ...
                        >
                           Course Exit Survey
                           {isExitCompleted && <CheckCircle size={14} className="inline ml-2 text-emerald-500 mb-0.5" />}
                        </button>
                     </div>
                  )}
                  {(!exitForm || activeFormTab === "faculty") ? renderFacultyEvaluation() : renderExitSurvey()}
                </div>
              );
            })()}
          </Card>
